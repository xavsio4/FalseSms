  ...
  <div class="col-md-6">
    ...
    <div class="row">
      <div class="col-md-6">
        ...
        <input  class="form-control input-sm" size="10" type="text" v-model="texters[0]"/>
        <p class="message message-in"><strong>{{texters[0] | capitalize}}</strong></p>
      </div>

      <div class="col-md-6">
        <label>Texter 2</label>
        <input class="form-control input-sm" size="20"  type="text" v-model="texters[1]" />
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body" id="chatwindow">
            ...
            <div v-for="msg in chat" class="row">
                <span v-bind:class="[(msg.from === 0) ?  'author-left pull-left' : 'author-right pull-right']" >{{texters[msg.from]}}</span>
                <div v-bind:class="['message', (msg.from === 0) ? 'message-in pull-left' : 'message-out pull-right']">
                    {{msg.message}}
                </div>
            </div>
            ...
            <form>
                <div class="input-group">
                  <span class="input-group-btn">
                      <a href="#" @click.prevent="switchAuthor(0)" v-bind:class="['btn btn-default', (this.swfrom === 0) ? 'selected-author' : '']">{{texters[0]}}</a>
                      <a href="#" @click.prevent="switchAuthor(1)" v-bind:class="['btn btn-default', (this.swfrom === 1) ? 'selected-author' : '']">{{texters[1]}}</a>
                  </span>
                  <input autofocus autocomplete="off"
                        placeholder="What needs to be done?"
                        v-model="newMsg"
                        @keyup.enter.prevent="addMsg" type="text" 
                        class="form-control">

                  <span class="input-group-btn">
                    <a href="#" @keyup.enter.prevent="addMsg" @click.prevent="addMsg"  class="btn btn-default" ><span class="glyphicon glyphicon-send"></span></a>
                  </span>
                </div>
            </form>
        </div>
...
export default {
  ...
  data: function ()
  {
      return {
        ...
        texters:['john','paty'],
        swfrom : 0,
        chat:[{'from':0,'message':'Hello'},{'from':1,'message':'Hi'}],
        }
  },
  props: {
    actionUrl: String,
  ...
  methods : 
  {
      ...
      {
        var elem = document.querySelector("#chatwindow")
        domtoimage.toJpeg(document.getElementById('chatwindow'), { quality: 0.95 })
